...
function Notifier() {
  ...
  this._putNotification = async function(data) {
    ...
    if (!utils.validateNotificationOptions(data, notificationOptions)) {
      throw new Error(`At least one of these notification options is required: (${notificationOptions.join(', ')})`)
    }

    const validatedData = utils.validateAndSanitizeData(data)
    ...
    if (validatedData.email && validatedData.email.template) {
